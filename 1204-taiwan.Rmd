...
srun -t 72:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -nthreads 12 \
-minInd 93 -bam bamlists/103.bamlist -ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
-rf  meta/no-inv.txt -out outputs/1204/snps-wgs-01  \
-minMaf 0.01 -minMapQ 20 -minQ 20 -GL 1 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doGeno 2 -doPost 1 -postCutoff 0.9 -doPlink 2  > outputs/1204/snps-wgs-01.out 2> outputs/1204/snps-wgs-01.err &

#creating GLF
srun -t 72:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -nthreads 12 \
-minInd 93 -bam bamlists/103.bamlist -ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
-rf  meta/no-inv.txt -out outputs/1204/snps-wgs-01-glf  \
-minMaf 0.01 -minMapQ 20 -minQ 20 -GL 1 -doGLF 2 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doGeno 2 -doPost 1 -postCutoff 0.9 -doPlink 2  > outputs/1204/snps-wgs-01-glf.out 2> outputs/1204/snps-wgs-01-glf.err &
...
```{r}
#Matt asked for 
#1 One for SEAsia plus Oceanpick 51 samples
m103 %>% filter(Lineage %in% c("SEA","OceanPick")) %>% select(NewPath) %>% write_tsv("bamlists/sea-oceanpick.bamlist", col_names = FALSE)
m103 %>% filter(Lineage %in% c("SEA","OceanPick")) %>% select(Run) %>% write_tsv("bamlists/sea-oceanpick.names", col_names = FALSE)

#2 IO plus Oceanpick? 33 samples

m103 %>% filter(Lineage %in% c("IND","lakdiva","OceanPick")) %>% select(NewPath) %>% write_tsv("bamlists/ind-oceanpick.bamlist", col_names = FALSE)
m103 %>% filter(Lineage %in% c("IND","lakdiva","OceanPick")) %>% select(Run) %>% write_tsv("bamlists/ind-oceanpick.names", col_names = FALSE)

#3 SEA
m103 %>% filter(Lineage %in% c("SEA")) %>% select(NewPath) %>% write_tsv("bamlists/sea41.bamlist", col_names = FALSE)
m103 %>% filter(Lineage %in% c("SEA")) %>% select(Run) %>% write_tsv("bamlists/sea41.names", col_names = FALSE)
...
#creating GLF
srun -t 72:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -nthreads 12 \
-minInd 46 -bam bamlists/sea-oceanpick.bamlist -ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
-rf  meta/no-inv.txt -out outputs/1204/glf-01-sea-oceanpick  \
-minMaf 0.01 -minMapQ 20 -minQ 20 -GL 1 -doGLF 2 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doGeno 2 -doPost 1 -postCutoff 0.9 -doPlink 2  > outputs/1204/glf-01-sea-oceanpick.out 2> outputs/1204/glf-01-sea-oceanpick.err &

srun -t 72:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -nthreads 12 \
-minInd 30 -bam bamlists/ind-oceanpick.bamlist -ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
-rf  meta/no-inv.txt -out outputs/1204/glf-01-ind-oceanpick \
-minMaf 0.01 -minMapQ 20 -minQ 20 -GL 1 -doGLF 2 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doGeno 2 -doPost 1 -postCutoff 0.9 -doPlink 2  > outputs/1204/glf-01-ind-oceanpick.out 2> outputs/1204/glf-01-ind-oceanpick.err &

srun -t 72:00:00 -p high --mem=32G --nodes=1 --ntasks-per-node=1 --cpus-per-task=12 $HOME/angsd/angsd -nthreads 12 \
-minInd 37 -bam bamlists/sea41.bamlist -ref genome/GCF_001640805.2_TLL_Latcal_v3_genomic.fna \
-rf  meta/no-inv.txt -out outputs/1204/glf-01-sea41 \
-minMaf 0.01 -minMapQ 20 -minQ 20 -GL 1 -doGLF 2 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doGeno 2 -doPost 1 -postCutoff 0.9 -doPlink 2  > outputs/1204/glf-01-sea41.out 2> outputs/1204/glf-01-sea41.err &
...
plink --tped snps-wgs-01.tped --tfam snps-wgs-01.tfam  --out plink-binary-01 --recode --allow-extra-chr --noweb
...
bcftools reheader --samples bamlists/103-sample-names.txt -o outputs/1204/renamed-01.vcf.gz outputs/1204/plink-01.vcf.gz
...
#Using random seed: 1738276351
bcftools +fill-tags outputs/1204/renamed-01.vcf.gz  -- -t MAF,F_MISSING | bcftools view -i 'F_MISSING < 0.05' | bcftools view -i 'INFO/MAF > 0.01' | bcftools view -i 'INFO/MAF < 0.45' |  bcftools +prune -m 0.20 -w 50kb --nsites-per-win 1 --nsites-per-win-mode rand  > outputs/1204/pruned-01-maf01-45.vcf

#Using random seed: 1739334052
bcftools +fill-tags outputs/renamed-01.vcf.gz  -- -t MAF,F_MISSING | bcftools view -i 'F_MISSING < 0.05' | bcftools view -i 'INFO/MAF > 0.10' | bcftools view -i 'INFO/MAF < 0.45' |  bcftools +prune -m 0.20 -w 50kb --nsites-per-win 1 --nsites-per-win-mode rand  > outputs/1204/pruned-01-maf10-45.vcf


#Using random seed: 1739335174
bcftools +fill-tags outputs/renamed-01.vcf.gz  -- -t MAF,F_MISSING | bcftools view -i 'F_MISSING < 0.05' | bcftools view -i 'INFO/MAF > 0.05' | bcftools view -i 'INFO/MAF < 0.45' |  bcftools +prune -m 0.20 -w 50kb --nsites-per-win 1 --nsites-per-win-mode rand  > outputs/1204/pruned-01-maf05-45.vcf
...
dat<-read.dna(file="outputs/1204/pruned-01-maf01-45.min4.phy")
write.nexus.data(dat, file="outputs/1204/pruned-01-maf01-45.nex")
...
ggplot(pcadata) +
  ...
  scale_shape_manual(values=c(21,23,23,22,22,24)) + 
...
iqtree2 -s pruned-01-maf01-45.min4.phy -st DNA -m GTR+ASC -bb 1000 --redo
iqtree2 -nt AUTO -s pruned-01-maf01-45.min4.phy.varsites.phy -T AUTO -st DNA -m MFP+ASC -bb 10000 --redo
(only 1461 variant sites)
...
net<-read.nexus.networx("outputs/1204/pruned-01-maf01-45-network.nex")
...
ggsave("outputs/1204/wgs-network-ind-id.jpeg", width=12, height=8)
ggsave("outputs/1204/wgs-network-ind-id.pdf", width=12, height=8)

```


```{r}
tree<-read.tree("outputs/1204/pruned-01-maf01-45.min4.phy.varsites.phy.contree")
t1<-ggtree(tree) + geom_nodelab(aes(label=node))
t1
```
...
```{r}
m<-ms
vcf <-read.vcfR("outputs/1204/pruned-01-maf01-45.vcf.gz") # from recalled and pruned snps in 1200
#vcf <-read.vcfR("outputs/1204/pruned-01-maf10-45.vcf") # getting a higher maf
...
#gl2<-gl.filter.callrate(glx,threshold=0.95, mono.rm = TRUE)
gl2<-glx
...
```{r}
edf<-structure$K_plot %>% as_tibble()

ggplot(edf) +
  geom_line(aes(x=k, y=Cross.Entropy)) +
  theme_bw() +
  ylab("Cross Entropy") +
  xlab("K Genetic Clusters") +
  theme(panel.grid = element_blank()) +
  scale_x_continuous(limits = c(2,6), breaks = c(2,3,4,5,6)) +
  theme(axis.title = element_text(size=14))

ggsave("outputs/1204/all-samples-genome-wide-admix-plot-entropy-scores.pdf")
```
...
write_csv(np %>% filter(K == "K = 4"), file="outputs/1204/k4.csv")
write_csv(np %>% filter(K == "K = 5"), file="outputs/1204/k5.csv")

np2<-np %>% filter(pop %in% c("OceanPick","Taiwan","Cambodia","Thailand","Philippines")) 

labs<-np2 %>% select(ID,sampID) %>% unique()

ggplot(np2 %>% filter(K == "K = 4")) +
  geom_bar(aes(x=ID, y=Percentage, fill=Cluster), stat = 'identity') +
...
ggsave("outputs/1204/k4.pdf", width=18, height=8)
...
snpad@snp.meta<-snpad@snp.meta %>% mutate(chr=gsub("_\\d+$","",snpID))  %>%
  mutate(position=gsub("NC_.+\\.\\d_","",snpID))
snpad@snp.meta$position<-as.numeric(snpad@snp.meta$position)
snpad@snp.meta
```

```{r}
admix<-plot_structure(snpad,  method = "admixture", admixture_path = "/Users/mac/github/admixture/")
admix$plot

```
method = "admixture",  admixture_path="/Users/mac/github/admixture/admixture", facet=c("pop"), facet.order = c("Queensland","Northern-Territory","Papua-New-Guinea","Mainstream","OceanPick",
                                           "Indonesia-K","Indonesia-SU","Indonesia-SJ","Philippines","Taiwan",
                                              "Vietnam","Cambodia","Thailand", "Bangladesh","Sri-Lanka",
                                              "India-East-Coast","India-West-Coast"),
               k=c(1,2,3,4,5,6), )
