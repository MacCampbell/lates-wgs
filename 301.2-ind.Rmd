...
ind<-meta %>% filter(Lineage=="IND")
ind %>% select(Run) %>% write_tsv("outputs/301/ind.samples", col_names = FALSE)
```

```{sh, eval=FALSE}
bcftools view -S outputs/301/ind.samples outputs/108/pruned.vcf | bcftools +fill-tags | bcftools view -q 0.05:minor > outputs/301/ind-05.vcf

```

```{r}
path_to_file <- "outputs/301/ind-05.vcf"
...
pcb<-ggplot(pcadata) +
  ...
  scale_fill_manual(values=cols[11:12]) +
...
ggsave("outputs/301/pcadapt-ind.jpeg", width=8.5, height=5.5)
