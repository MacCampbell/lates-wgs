...
0.230  -0.211   -0.0361  0.795    WAF0… WAF003776
...
bcftools view -S bamlists/89.names -o outputs/1400/renamed-01.vcf.gz outputs/1400/renamed-01-90.vcf.gz
...
```
```{r}
m89 %>% group_by(Region, Locality) %>% summarize(Count=n(), Median=median(Filtered), Median=median(Coverage) )
```
...
path_to_file <- "outputs/1400/pruned-01-maf05-45.vcf"
...
ggsave("outputs/1400/called-snp-pcas.pdf", width=12, height=10)
```
...
iqtree2  -nt AUTO -m MFP+ASC -bb 10000 -s  pruned-01-maf05-45.min4.phy.varsites.phy --seqtype DNA --redo
...
```{r}
dat<-read.dna(file="outputs/1400/phylo/pruned-01-maf05-45.min4.phy.varsites.phy")
write.nexus.data(dat, file="outputs/1400/phylo/pruned-01-maf05-45.min4.phy.varsites.nex")


dat<-read.dna(file="outputs/1400/phylo/pruned-01-maf01-45.min4.phy.varsites.phy")
write.nexus.data(dat, file="outputs/1400/phylo/pruned-01-maf01-45.min4.phy.varsites.nex")
```



```{r}
net<-read.nexus.networx("outputs/1400/maf05.network")
m<-m89
```
...
#  theme(legend.position = "none") +
  theme(panel.grid=element_blank()) +
  geom_tiplab2(aes(label=Locality), size=2)
...
n
ggsave("outputs/1400/wgs-network.jpeg", width=10, height=8)
ggsave("outputs/1400/wgs-network.pdf", width=10, height=8)

```
```{r}
tree<-read.tree("outputs/1400/phylo/pruned-01-maf05-45.min4.phy.varsites.phy.contree")
...
ggsave("outputs/1400/phylo.pdf", width=7, height=9)
```