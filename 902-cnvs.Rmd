...
```{r}
commands<-msub %>% mutate(Command=paste0("samtools view -bh -r NC_066833.1,NC_066834.1 ", Path, " | samtools depth -a - | awk '{print $1, $2, $3, $3/",Coverage,"}' ", ">", " outputs/902/",Run,".coverage"))
...
srun -p high --time 12:00:00 --nodes=1 --ntasks-per-node=1 --cpus-per-task=18 parallel -j 18 < 902-coverage.sh
