...
-out outputs/1100/60-ibs-90 > outputs/1100/60-ibs-90.out 2> outputs/1100/60-ibs-90.err &
	

srun -p high -t 04:00:00 --mem=32G --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/1100/60-ibs-90.beagle.gz -K 2 -o outputs/1100/60-K2 &
srun -p high -t 05:00:00 --mem=32G --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/1100/60-ibs-90.beagle.gz -K 3 -o outputs/1100/60-K3 &
srun -p high -t 08:00:00 --mem=32G --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/1100/60-ibs-90.beagle.gz -K 4 -o outputs/1100/60-K4 &
srun -p high -t 08:00:00 --mem=32G --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/1100/60-ibs-90.beagle.gz -K 5 -o outputs/1100/60-K5 &
srun -p high -t 09:00:00 --mem=32G --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/1100/60-ibs-90.beagle.gz -K 6 -o outputs/1100/60-K6 &
...
-minMaf 0.05 \
-out outputs/1100/60-gls-90 > outputs/1100/60-gls-90.out 2> outputs/1100/60-gls-90.err &
...
```{r}
m <- as.matrix(read.table("outputs/1100/60-ibs-90.covMat"))
eig <- eigen(m)
...
ggsave("outputs/1100/lates-c-genome-wide-pca-12.jpeg")
```
For figure  
...
ggsave("outputs/1100/combined-wgspcs.pdf", width=11, height=5)
ggsave("outputs/1100/combined-wgspcs.jpeg", width=11, height=5)
...
```{r}
PlotAdmix("outputs/1100/60-K2.qopt", meta)
```


```{r}
PlotAdmix("outputs/1100/60-K3.qopt", meta)
```
```{r}
PlotAdmix("outputs/1100/60-K4.qopt", meta)
```

```{r}
PlotAdmix("outputs/1100/60-K5.qopt", meta)
```
...
k2<-PlotAdmixNoLabs2("outputs/1100/60-K2.qopt", meta)
k2
```
...
k3<-PlotAdmixNoLabs3("outputs/1100/60-K3.qopt", meta)
k3
```
...
k4<-PlotAdmixNoLabs4("outputs/1100/60-K4.qopt", meta)
k4
```
...
k5<-PlotAdmix5("outputs/1100/60-K5.qopt", meta)
k5
```
...
ggsave("outputs/1100/combined-k-plots.pdf", width=11, height=8.5)
```
...
-rf meta/no-inv.txt -out outputs/1100/snps-wgs  \
-minMaf 0.05 -minMapQ 20 -minQ 20 -GL 1 -doMajorMinor 1 -doMaf 1 -SNP_pval 1e-6 \
-doGeno 2 -doPost 1 -postCutoff 0.9 -doPlink 2  > outputs/1100/snps-wgs.out 2> outputs/1100/snps-wgs.err &

```
	-> Number of sites retained after filtering: 3327142 
	-> After removal of inversion regions
...
#renaming vcf
bcftools reheader --samples bamlists/60-sample-names.txt -o outputs/1100/renamed.vcf.gz outputs/1100/plink.vcf.gz

bcftools +prune -l 0.20 -w 10000 outputs/1100/renamed.vcf.gz > outputs/1100/pruned.vcf
bcftools +prune -l 0.30 -w 10000 outputs/1100/renamed.vcf.gz > outputs/1100/pruned-03.vcf
```
