...
pcadf$Region<-factor(pcadf$Region, levels=c("AUE","Queensland","AUW","NorthernTerritory","PNG","PapuaNewGuinea","INA","Indonesia-K","Indonesia-SJ","Indonesia-SU",
                                               "MAL","Cambodia","THA","Thailand","Vietnam","Philippines","Bangladesh","IndiaEasternCoast","IndiaWesternCoast"))
```
...
```{r}
ggplot(pcadf) +
  geom_point(aes(x=PC1, y=PC2, fill=Region, shape=Lineage), alpha=0.75, cex=2) +
  geom_text_repel(data=labels, aes(x=x, y=y, label=Lineage), face="bold", size=6)+
  theme_bw() +
  theme(panel.grid = element_blank()) +
...
bcftools view -r NC_066837.1:5247652-28551740 outputs/607/snps-05.vcf.gz -O z > outputs/607/NC_066837.1-sub.vcf;    
