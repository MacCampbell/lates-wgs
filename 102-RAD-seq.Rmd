...
#ngs admix    
srun -p high -t 01:00:00 --mem=32G --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/102/oz-ibs.beagle.gz -K 2 -o outputs/101/oz-K2 &
srun -p high -t 01:00:00 --mem=32G --nodes=1 $HOME/angsd/misc/NGSadmix -P 12 -likes outputs/102/oz-ibs.beagle.gz -K 3 -o outputs/101/oz-K3 &
...
```{r}
pcm<-oz
m <- as.matrix(read.table("outputs/102/oz-ibs-90-maf1.covMat"))
eig <- eigen(m)
...
```{r}
covs<-eig$vectors[,1:3] %>% as_tibble() %>% bind_cols(pcm) %>% filter(Isolate %in% samples$Isolate)
...
```
There is a mismatch between what is reported and what there is. One AUW is dropped and not reported in the genepop file.    
