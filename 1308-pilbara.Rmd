...
m129 %>% dplyr::select(NewPath) %>% write_tsv("bamlists/129.bamlist", col_names = FALSE)
m129 %>% dplyr::select(Run) %>% write_tsv("bamlists/129.names", col_names = FALSE)

m69<-m129 %>% filter(Source=="New")
m69 %>% dplyr::select(NewPath) %>% write_tsv("bamlists/69.bamlist", col_names = FALSE)
m69 %>% dplyr::select(Run) %>% write_tsv("bamlists/69.names", col_names = FALSE)

aus<-m129 %>% filter(Lineage=="AUS")

aus %>% dplyr::select(NewPath) %>% write_tsv("bamlists/61.bamlist", col_names = FALSE)
aus %>% dplyr::select(Run) %>% write_tsv("bamlists/61.names", col_names = FALSE)
...
m76 %>% dplyr::select(NewPath) %>% write_tsv("bamlists/76.bamlist", col_names = FALSE)
m76 %>% dplyr::select(Run) %>% write_tsv("bamlists/76.names", col_names = FALSE)


m65<-bind_rows(aus,w) %>%  filter(Run !="WAF011639") %>% filter(Run != "Broome01") %>% write_csv(file="meta/m65.csv")
m65 %>% dplyr::select(NewPath) %>% write_tsv("bamlists/65.bamlist", col_names = FALSE)
m65 %>% dplyr::select(Run) %>% write_tsv("bamlists/65.names", col_names = FALSE)
...
bcftools view -r NC_066847.1:13250000-13450000 outputs/1308/renamed-01.vcf.gz > outputs/1308/lca15-region1.vcf

m76
bcftools view -r NC_066847.1:13250000-13450000 outputs/1308/renamed-01.vcf.gz | bcftools view -S bamlists/76.names  > outputs/1308/lca15-region1.vcf


```{r}

vcf<-read.vcfR(file="outputs/1308/lca15-region1.vcf")

genind<-vcfR2genind(vcf)
genind@pop<-as.factor(m76$Locality)
...
pc12<-ggplot(df) + 
  geom_point(aes(x=Axis1, y=Axis2, fill=Lineage, shape=Lineage), alpha=0.75, cex=2) +
  theme_bw() +
  xlab(paste0("PC1 ",round(eig[1],2), "%")) +
  ylab(paste0("PC2 ",round(eig[2],2), "%")) +
  theme(panel.grid = element_blank()) +
...
ggsave("outputs/1309/lca15-region-1-pca.jpeg", width=6, height=8.5)
