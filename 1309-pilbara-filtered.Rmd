...
colors<-viridis(13,option="H")
colors<-colors[1:11]

meta %>% group_by(Locality) %>% summarize(Count=n())
...
pcb<-ggplot(pcadata) +
  ...
  theme(legend.position = "none") +
  theme(panel.grid = element_blank()) +
  xlab(paste0("PC 1 (",var[1],")%")) +
  ylab(paste0("PC 2 (",var[2],")%")) +
  ggtitle("B") +
  theme(plot.title = element_text(size=14, face="bold")) 
...
pcc<-ggplot(pcadata) +
  ...
  xlab(paste0("PC 1 (",var[1],")%")) +
  ylab(paste0("PC 3 (",format(var[3], nsmall = 2),")%")) +
  ggtitle("C") +
  theme(plot.title = element_text(size=14, face="bold")) 
...
bcftools view -r NC_066847.1:13250000-13450000 outputs/1309/renamed-05.vcf.gz > outputs/1309/lca15-region1.vcf



```{r}
vcf<-read.vcfR(file="outputs/1309/lca15-region1.vcf")
...
pc12<-ggplot(df) + 
  geom_point(aes(x=Axis1, y=Axis2, fill=Locality, shape=Lineage), alpha=0.75, cex=2) +
  theme_bw() +
