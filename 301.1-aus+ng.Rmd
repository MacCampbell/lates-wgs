...
moz<-meta %>% filter(Lineage=="AUS+NG")
moz %>% select(Run) %>% write_tsv("outputs/301/ozng.samples", col_names = FALSE)
```

```{sh, eval=FALSE}
bcftools view -S outputs/301/ozng.samples outputs/108/pruned.vcf | bcftools +fill-tags | bcftools view -q 0.05:minor > outputs/301/oz-ng-05.vcf

```

```{r}
path_to_file <- "outputs/301/oz-ng-05.vcf"
...
ggsave("outputs/301/pcadapt-ozng.jpeg", width=8.5, height=5.5)
